...
layout(std430, binding=4) buffer IntersectionsBuffer {
    float intersectionsTimes[];
};

uniform float fpTol;

uint intersectionCount = 0;
void addTime(float t, uint intersectionsIndex) {
    uint i = intersectionCount;
    while (i > 0) {
        if (intersectionsTimes[intersectionsIndex + i - 1] <= t)
            break;
    ...
    intersectionsTimes[intersectionsIndex + i] = t;
    intersectionCount++;
}
