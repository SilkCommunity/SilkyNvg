...
namespace SilkyNvg
{
    ...
    public class Path
    {
        ...
        private readonly List<IPathCommand> _pathCommands = new List<IPathCommand>();

        // If a path is not modified between filling / stroking calls, then we can render these in the same GPU call
        internal bool Modified { get; set; } = true;

        internal IEnumerator<IPathCommand> GetCommandEnumerator() => _pathCommands.GetEnumerator();
